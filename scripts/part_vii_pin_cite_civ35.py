#!/usr/bin/env python3
"""Insert transcript ### section anchors for civ-35 and refresh L2 refs."""
...
# Lecture-order sections (claim rows map by slug, not table order).
...
def update_readiness() -> None:
    ...
    text = text.replace(
        "| Pin-cite `civ-35`–`41` | **Wedge done** — `civ-40` + `civ-41`; `civ-35`–`39` **deferred** |",
        "| Pin-cite `civ-35`–`41` | **Partial** — `civ-35` + wedge `civ-40`/`civ-41`; `civ-36`–`39` **deferred** |",
    )
    if "| Pin-cite `civ-35` |" not in text:
        needle = "| Pin-cite `civ-40` | **Done** — `scripts/part_vii_pin_cite_civ40.py` |"
        if needle in text:
            text = text.replace(
                needle,
...
def main() -> None:
    ...
    update_readiness()
    print("part_vii_pin_cite_civ35: done")
